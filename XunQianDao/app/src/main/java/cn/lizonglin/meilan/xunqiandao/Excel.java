...
public class Excel{
    ...
    public static void initExcel(String fileName, String[] colName) {
        ...
        try {
            ...
            WritableSheet sheet = workbook.createSheet("学生签到表", 0);
    ...
    public static <T> void writeObjListToExcel(List<T> objList,
        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                writebook.write();
                Toast.makeText(c, "导出到手机存储中文件夹Course成功", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                if (writebook != null) {
    ...
    public static  List<CheckObject> read2DB(File f, Context con) {
        ArrayList<CheckObject> stuList = new ArrayList<CheckObject>();
        try {
            ...
            for (int i = 1; i < sheet.getRows(); i++) {
                CheckObject tc = new CheckObject();
                cell = sheet.getCell(1, i);
                tc.setStuid(cell.getContents());
                cell = sheet.getCell(2, i);
                tc.setClasstime(cell.getContents());
                cell = sheet.getCell(3, i);
                tc.setMajor(cell.getContents());
                cell = sheet.getCell(4, i);
                tc.setStatus(cell.getContents());
                Log.d("nothing", "Row"+i+"---------"+tc.getStuid() + tc.getClasstime()
                        + tc.getMajor() + tc.getStatus());
                stuList.add(tc);
            }
